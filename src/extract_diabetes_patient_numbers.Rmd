...
```{sql, connection=c_connect, output.var = "tst0"}
SELECT DISTINCT concept_cd 
FROM blueherondata.concept_dimension
WHERE concept_cd LIKE '%ICD9:250%'
```
...
```{sql, connection=c_connect, output.var = "tst0"}
CREATE OR REPLACE VIEW diabetes_icd9 AS
SELECT DISTINCT concept_cd 
FROM blueherondata.concept_dimension
WHERE concept_cd LIKE '%ICD9:250%'
```
...
```{sql, connection=c_connect, output.var = "tst1"}
SELECT * FROM diabetes_icd9
```
...
dbSendQuery(c_connect, sql_view)
```
